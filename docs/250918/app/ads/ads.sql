...
where ds>=date_add('2025-09-17',-recent_days+1)
...
from
    (
        ...
        where ds='2025-09-17'
          and login_date_last=date_add('2025-09-17',-7)
    )churn
        join
    (
        select
        ...
        from
            (
                ...
                from dws_user_user_login_td
                where ds='2025-09-17'
                  and login_date_last = '2025-09-17'
            )t1
                join
            (
                select
                ...
                from dws_user_user_login_td
                where ds=date_add('2025-09-17',-1)
            )t2
...
union
select '2025-09-17' ds,
       login_date_first create_date,
       datediff('2025-09-17', login_date_first) retention_day,
       sum(if(login_date_last = '2025-09-17', 1, 0)) retention_count,
       count(*) new_user_count,
       cast(sum(if(login_date_last = '2025-09-17', 1, 0)) / count(*) * 100 as decimal(16, 2)) retention_rate
from (
         select user_id,
                login_date_last,
                login_date_first
         from dws_user_user_login_td
         where ds = '2025-09-17'
           and login_date_first >= date_add('2025-09-17', -7)
           and login_date_first < '2025-09-17'
     ) t1
...
select '2025-09-17' ds,
       recent_days,
       sum(if(login_date_first >= date_add('2025-09-17', -recent_days + 1), 1, 0)) new_user_count,
       count(*) active_user_count
from dws_user_user_login_td lateral view explode(array(1, 7, 30)) tmp as recent_days
where ds = '2025-09-17'
  and login_date_last >= date_add('2025-09-17', -recent_days + 1)
...
where ds='2025-09-17'
  and order_date_first>=date_add('2025-09-17',-recent_days+1)
group by recent_days;
...
from
    (
        select
            user_id,
            datediff(lead(ds,2,'99991231') over(partition by user_id order by ds),ds) diff
        from dws_trade_user_order_1d
        where ds>=date_add('2025-09-17',-6)
    )t1
where diff=2;
...
where ds in ('99991231','2025-09-17')
  and payment_date_id='2025-09-17';
