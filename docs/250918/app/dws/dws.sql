...
set hive.vectorized.execution.enabled = false;
insert overwrite table dws_trade_user_sku_order_1d partition(ds='20250918')
select
    user_id,
    id,
    sku_name,
...
from
    (
        ...
        from dwd_interaction_favor_add
        group by ds,sku_id
    )favor
        left join
    (
...
insert overwrite table dws_trade_user_sku_order_nd partition(ds='20250917')
select
    user_id,
    sku_id,
    sku_name,
    ...
    tm_id,
    tm_name,
    sum(if(ds>=date_add('20250917',-6),order_count_1d,0)),
    sum(if(ds>=date_add('20250917',-6),order_num_1d,0)),
    sum(if(ds>=date_add('20250917',-6),order_original_amount_1d,0)),
    sum(if(ds>=date_add('20250917',-6),activity_reduce_amount_1d,0)),
    sum(if(ds>=date_add('20250917',-6),coupon_reduce_amount_1d,0)),
    sum(if(ds>=date_add('20250917',-6),order_total_amount_1d,0)),
    sum(order_count_1d),
    sum(order_num_1d),
...
insert overwrite table dws_trade_province_order_nd partition(ds='20250917')
select
    province_id,
    province_name,
    area_code,
...
insert overwrite table dws_trade_user_order_td partition(ds='20250917')
select
    user_id,
...
from (
         ...
         union all
         select user_id,
                '20250918' order_date_first,
                '20250918' order_date_last,
                order_count_1d,
                order_num_1d,
                order_original_amount_1d,
                activity_reduce_amount_1d,
